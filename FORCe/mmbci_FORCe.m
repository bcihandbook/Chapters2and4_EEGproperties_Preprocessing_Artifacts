function [hasArtifacts, cleanEEG, remICs] = mmbci_FORCe( EEGdata, Fs, chanLocs, useAcc, params, doclean )
    %
    % FORCe
    %
    ...
    %
    %    doclean    Boolean denoting whether data should be cleaned (1/true)
    %
    %    params     Struct with thresholding parameters
    % Output:
    %
    ...
    for tN = 1:length( tUse ),
        ...
        if tN == 1,% If this is the first terminal node (the approximation coefficients).
            ...
            remICs = find( lenINo > params.thfailedtests );
            
            if(length(remICs)/NCh > params.thfailedicprct)
                % Label this window as artifact inflicted when X% of
                % channels (ICs in fact) fail more than Y tests
                hasArtifacts = 1;
            else
                hasArtifacts = 0;
            end
        ...
        if(doclean) % We do not need those if we do not want artifact removal
    ...
    % If no need to clean, we can skip the rest and save some time
    if(doclean ~= 1)
        cleanEEG = NaN;
        return;
    end
